    ...
    private void UpdateWhenConnected()
    {
        ...
        if (simplePackets != null)
        {
            foreach (var packet in simplePackets)
            {
                ...
                // _logger.TraceFormat("OnSimple: Msg={0}", packet.MessageName);

                /*
                var idleEcho = packet.Message as PcIdleEcho;
                if (idleEcho != null)
                {
                    OnIdleEcho(idleEcho);
                    continue;
                ...
                var kickClient = packet.Message as PcKickClient;
                if (kickClient != null)
                {
                ...
                if (SimplePacketHandlerMap.TryGetValue(msg.GetType(), out handler))
                {
                    if (handler != null)
                        handler(msg);
                    continue;
                ...
                */
                ...
                _logger.WarnFormat("SimplePacket arrived but no handler: {0}", msg.GetType().Name);
            }
        }
